...
#                     pop_temp,div_temp = get_pop_div(date,sd,temp)
    ...
    if not plotting:
        ...
        ax[0,0].boxplot(pops_by_T[:],positions=positions,labels=xlabels,notch=True,showfliers=False)
        ax[0,0].set_ylabel(f"Total pop'ln",color="blue")
        ax[1,0].boxplot(divs_by_T[:],positions=positions,labels=xlabels,showfliers=False,notch=True)
        ax[1,0].set_ylabel(f"Total div.",color="blue")
        # ax[1,0].tick_params(labelrotation=90)
        
        ax[0,1].boxplot(core_pops_by_T[:],positions=positions,labels=xlabels,showfliers=False,notch=True)
        ax[1,1].boxplot(core_divs_by_T[:],positions=positions,labels=xlabels,showfliers=False,notch=True)
        ax[0,1].set_ylabel("Core pop'ln",color="b")
        ax[1,1].set_ylabel("Core div.",color="b")
        # ax[1,1].tick_params(labelrotation=90)
        
        ax[2,0].boxplot(interactions_by_T[:],positions=positions,labels=xlabels,showfliers=False,notch=True)
        ...
        ax[2,1].plot(T,TM.poff_total(0,T)-TM.pdeath(T))
        ...
        #try:
        plt.savefig(locat+experiment+date+extra_folder+"/fast_comparison_"+experiment[:-1]+date+".pdf")
        #except:
