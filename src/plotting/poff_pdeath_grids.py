...
pdeath_range = np.r_[.05:1:.05] #[.05:.45:.05,.5:1:.1] # [False] # np.r_[.1:1:.1]
poff_range = np.r_[.05:1:.05] # [False] # np.r_[.1:1:.1]
C = 100
mu = .05
theta = .25
pmut = .001
...
dates = ['Mar_15_23'] # ['Jan_18','Jan_31','Feb_06'] #,'Oct_11'] # ['Sep_09'] #['Jul_21'] # ['Jun_20'] # ['Mar_21','Mar_22'] #'Feb_28'
maxgens = 10_000
sample_size = 50
...
extra_folder = 'mu_.05/' #'/prob_test' #"/MTE-env" # "/prob_test" # "/MTE_TPC_combo" # "/Variable_Tresponse" # "/poff_is_roff" #"/poff_is_roff" #'/skew_norm_new_parameters' #/carb_truncated' #'/carb_model_branch' #'/Feb_21_copy_truncated/truncated' # slash in front only or else ''
L = 20 #20
...
def find_file(date,seed,T=False,poff=False,pdeath=False,type="pypy"):
    ...
    if type == "pypy": 
        ...
        if poff:
            if "kenes" in locat:
                file = f"{locat}{experiment}{extra_folder}{date}/pypy_seed{str(seed)}{date[:6]}_poff{poff:.2f}_pdeath{pdeath:.2f}K_TNM_C{C}_mu{mu:.2f}_theta{theta:.2f}_pmut{pmut:.3f}_L{L}.dat"
            else:
        ...
        if not exists(file):
            ...
            for j in range(0,num_tries): #,end): # np.r_[0:end]: #len(other_file_dates)]:
                ...
                if poff:
                    if "kenes" in locat:
                        file = f"{locat}{experiment}{extra_folder}{date}/pypy_seed{str(seed)}{date2}_poff{poff:.2f}_pdeath{pdeath:.2f}K_TNM_C{C}_mu{mu:.2f}_theta{theta:.2f}_pmut{pmut:.3f}_L{L}.dat"
                    else:
...
def get_pop_div(date,seed,T=False,poff=False,pdeath=False,type="pypy",filename=None):
...
#        print(exists(pypy_file))
        ...
        pypy_file,date2 = find_file(date,seed,T=T,poff=poff,pdeath=pdeath,type="pypy")
        print('************\n'+experiment+'\n',pypy_file)
    if pypy_file == 0:
        return 0,0,0,0,0
...
#        fig,ax = plt.subplots(len(poff_range),len(pdeath_range))
        ...
        if extra_folder.startswith("/"):
            x_fo_name = extra_folder[1:]
            x_name = experiment[1:]
        elif extra_folder.endswith("/"):
            x_fo_name = extra_folder[:-1]
            x_name = experiment[:-1]
        sfig.savefig(locat+experiment+date+extra_folder+"/survival_grid_"+x_name+date+x_fo_name+".pdf")
        afig.savefig(locat+experiment+date+extra_folder+"/abundance_grid_"+x_name+date+x_fo_name+".pdf")
        #plt.imsave(locat+experiment+date+extra_folder+"/abundance_imsave_"+experiment[:-1]+date+extra_folder[1:]+".pdf")
        dfig.savefig(locat+experiment+date+extra_folder+"/diversity_grid_"+x_name+date+x_fo_name+".pdf")
